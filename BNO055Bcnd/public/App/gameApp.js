angular.module("gameApp", ['btford.socket-io'])
    ...
    var myIoSocket = io.connect('http://pixi-yadunandan004.c9users.io:8080/');
     mySocket = socketFactory({
            ioSocket: myIoSocket
          });
        return mySocket;
        })
    .controller("appController", ["$scope", "keyPressHandlerService","socket", function($scope, keyPressHandlerService,socket) {
        $scope.keydown = function(keyEvent) {
            ...
            socket.emit('bdata',keyEvent.keyCode);
    ...
    .directive("centipedeGame", ["$interval", "gameService", "renderService", "graphicsEngineService", function($interval, gameService, renderService, graphicsEngineService) {
        return {
            restrict: 'A',
            template: '<canvas id="gameCanvas" width="600" height="640" style="border:1px solid #80CBC4;"></canvas>',
